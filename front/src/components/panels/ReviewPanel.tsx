...
function AgentReviewNotice({ result }: { result: ReviewResult }) {
  ...
  const heading =
    status === 'queued' ? 'Bedrock 판단 대기 중'
    : status === 'running' ? 'Bedrock에서 판단 중'
    : status === 'completed' ? 'Bedrock 판단 완료'
    : status === 'failed' ? 'Bedrock 판단 실패'
    : status === 'blocked' ? 'Bedrock 판단이 차단됨'
    : `AgentCore Reviewer: ${formatAgentStatus(status)}`
  const message =
    result.agent_review_message ||
    (status === 'queued'
      ? 'LLM/hybrid 규칙 판단 요청을 AgentCore Runtime으로 보냈습니다. 곧 Bedrock에서 평가가 시작됩니다. 먼저 정적 규칙 결과를 표시합니다.'
      : status === 'running'
        ? 'AgentCore Runtime이 Bedrock을 호출해서 LLM/hybrid 규칙을 평가하고 있습니다. 완료되면 자동으로 결과가 갱신됩니다. (보통 30초~2분)'
        : status === 'completed'
          ? 'AgentCore Reviewer가 LLM/hybrid 규칙 판단을 완료했습니다.'
          : 'AgentCore Reviewer 결과를 가져올 수 없습니다.')
  return (
    <div
    ...
    >
      <strong>
        {isPending ? (
          <>
            <span className="bedrock-spinner" aria-hidden="true" /> {heading}
          </>
        ) : (
          heading
        )}
