  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: space.md }}>
      ...
      <div style={{
        ...
        <Metric label="Target Funding" value={formatUsd(result.target_funding_amount ?? 0)} />
        <Metric label="Required ARR" value={formatUsd(result.required_arr)} />
        <Metric label="SOW Cost Requirement" value={formatUsd(result.sow_cost_requirement)} />
        <Metric label="$125K Cap Check" value={capLimited ? `⚠ Limited (${formatUsd(cap)})` : `OK (${formatUsd(cap)})`} warn={capLimited} />
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
        <button
          ...
          style={{
            fontSize: 12,
            background: crState.state === 'created' ? color.success : undefined,
            borderColor: crState.state === 'created' ? color.success : undefined,
          }}
          title={
            fallback
...
function Metric({ label, value, highlight, warn }: { label: string; value: string; highlight?: boolean; warn?: boolean }) {
  const cls = warn ? 'metric-card is-warning' : highlight ? 'metric-card is-highlight' : 'metric-card'
  return (
    <div className={cls}>
