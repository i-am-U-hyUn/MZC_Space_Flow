      ...
      {onApprove && onReject && cr.status === 'pending' && (
        <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
          ...
          <button
            onClick={onReject}
            disabled={actionInFlight}
            className="mzc-btn mzc-btn-danger"
            style={{ fontSize: 11 }}
          >
            ✗ Reject
          </button>
...
function ChangeEntry({ change }: { change: ChangeRequestChange }) {
  return (
    <div style={{ marginTop: 8, padding: 10, background: color.bgSubtle, borderRadius: radius.sm, fontSize: 11, border: `1px solid ${color.border}` }}>
      {change.section && (
        <div style={{ fontWeight: 700, marginBottom: 6, color: color.textPrimary, fontSize: 12 }}>{change.section}</div>
      )}
      <div className="diff-block">
        <span style={{ color: color.error, fontWeight: 700, fontSize: 10, letterSpacing: '0.05em' }}>AS-IS</span>
        <div className="diff-before">{truncate(stringify(change.as_is))}</div>
        <span style={{ color: color.success, fontWeight: 700, fontSize: 10, letterSpacing: '0.05em' }}>TO-BE</span>
        <div className="diff-after">{truncate(stringify(change.to_be))}</div>
      </div>
      {change.reason && (
        <div style={{ marginTop: 6, color: color.textSecondary, fontStyle: 'italic' }}>
          이유: {change.reason}
        </div>
      )}
    </div>
