        ...
        {items.map(preset => (
          <button
            ...
            style={{
              ...
              textAlign: 'left',
              borderRadius: 10,
              border: `1px solid ${selectedId === preset.id ? color.primary : color.border}`,
              background: selectedId === preset.id ? color.primarySoft : color.bgSurface,
              cursor: 'pointer',
              fontSize: 12,
              fontWeight: 600,
              color: selectedId === preset.id ? color.primary : color.textPrimary,
              boxShadow: selectedId === preset.id ? '0 0 0 3px rgba(0, 87, 255, 0.08)' : 'none',
              transition: 'border-color 120ms ease, background 120ms ease',
            }}
          >
            <span className="mzc-badge mzc-badge-ai" style={{ marginBottom: 6 }}>Suggested</span>
            <div style={{ marginTop: 4 }}>{preset.label}</div>
          </button>
        ))}
      </div>
