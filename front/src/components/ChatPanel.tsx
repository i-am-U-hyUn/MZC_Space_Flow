...
const STYLE_ONCE_ID = '__spaceflow_thinking_style'
...
export function ChatPanel({ docId }: ChatPanelProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('spaceflow:chat-prompt', handler as EventListener)
    return () => window.removeEventListener('spaceflow:chat-prompt', handler as EventListener)
  }, [])
