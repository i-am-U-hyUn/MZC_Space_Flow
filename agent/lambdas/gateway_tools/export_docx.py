...
def _build_context(params: dict[str, Any]) -> dict[str, Any]:
    ...
    # --- Architecture diagram (v2): preview_s3_key 우선, 옛 diagram_image_s3_key
    # 호환. 값이 FieldValue wrapper면 풀어내고, 최종적으로 S3 key 문자열만 남김.
    # _render_docx가 이 key를 S3에서 다운로드 → InlineImage로 교체한다.
    preview_key = architecture.get("preview_s3_key", "")
    if isinstance(preview_key, dict):
        preview_key = resolve_field_value(preview_key, "")
    if not isinstance(preview_key, str):
        preview_key = ""

    if not preview_key:
        legacy = resolve_field_value(architecture.get("diagram_image_s3_key", ""))
        if isinstance(legacy, str):
            preview_key = legacy

    architecture_diagram_image = preview_key
...
def _register_document_namespaces(xml_bytes: bytes) -> None:
    """Register every xmlns:prefix declaration found in the document root.

    xml.etree.ElementTree reassigns unknown prefixes to ns0, ns1... on
    re-serialization, which breaks `mc:Ignorable` and other prefix-list
    attributes. Scanning the raw XML and pre-registering each prefix
    preserves the original prefixes across parse → serialize round-trips.
    """
    try:
        head = xml_bytes[:8192].decode("utf-8", errors="ignore")
    except Exception:
        return
    # Match xmlns:prefix="uri" declarations (skip the default xmlns="uri"
    # which ElementTree already handles).
    for prefix, uri in re.findall(r'xmlns:([A-Za-z_][\w.-]*)\s*=\s*"([^"]+)"', head):
        try:
...
def _postprocess_docx(docx_bytes: bytes, context: dict[str, Any]) -> bytes:
    ...
    _register_document_namespaces(files[document_xml])
...
def _prepare_diagram_image(doc: Any, s3_key: str) -> Any | None:
    """Download a diagram image from S3 and return a docxtpl InlineImage.

    Returns ``None`` if the key is empty, the object is missing, or any
    failure occurs. Caller should then treat the value as an empty string
    so the template's ``{%p if architecture_diagram_image %}`` branch skips.
    """
    ...
    try:
        ...
        resp = s3.get_object(Bucket=ARTIFACTS_BUCKET, Key=s3_key)
        body = resp["Body"].read()
    except Exception as exc:  # noqa: BLE001
        print(f"[export_docx] diagram download failed key={s3_key} err={type(exc).__name__}: {exc}")
        return None
    ...
    if lower.endswith(".jpg") or lower.endswith(".jpeg"):
        ...
        # docxtpl/python-docx do not render SVG natively. Skip rather than crash.
        print(f"[export_docx] SVG diagram not supported as inline image key={s3_key}")
        return None

    # Write to a temp file; python-docx reads by path.
    tmp = tempfile.NamedTemporaryFile(suffix=suffix, delete=False)
    try:
        tmp.write(body)
        tmp.flush()
        tmp.close()
        # 150mm ~ fits inside the A4 body with the template's side margins.
        return InlineImage(doc, tmp.name, width=Mm(150))
    except Exception as exc:  # noqa: BLE001
        print(f"[export_docx] InlineImage construction failed key={s3_key} err={type(exc).__name__}: {exc}")
        return None
...
def _render_docx(template_bytes: bytes, context: dict[str, Any]) -> bytes:
    ...
    with tempfile.NamedTemporaryFile(suffix=".docx") as template_file:
        ...
        # Replace the S3 key string with an InlineImage so the diagram
        # actually renders in the exported DOCX. If anything goes wrong
        # (missing object, SVG, download failure), fall back to empty
        # string — the template's `{%p if %}` branch hides the block.
        diagram_key = context.get("architecture_diagram_image") or ""
        image = _prepare_diagram_image(doc, diagram_key) if isinstance(diagram_key, str) else None
        context = dict(context)
        context["architecture_diagram_image"] = image if image is not None else ""
