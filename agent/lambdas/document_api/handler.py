...
_agentcore_fire_and_forget_client = None


def _get_agentcore_runtime_ff_client():
    """Runtime client with a short read timeout.

    Used for fire-and-forget review invocations where the Runtime writes
    results directly to DynamoDB and the Lambda should return quickly without
    holding the HTTP connection open for the full Bedrock judgment loop.
    """
    global _agentcore_fire_and_forget_client
    if _agentcore_fire_and_forget_client is None:
        from botocore.config import Config
        _agentcore_fire_and_forget_client = boto3.client(
            "bedrock-agentcore",
            region_name=REGION,
            config=Config(
                connect_timeout=5,
    ...
    return _agentcore_fire_and_forget_client
...
def _save_review_job(job: dict) -> None:
    ...
    # user_id를 GSI(`user_id-updated_at-index`) 파티션 키에 그대로 두면
    # 사이드바 문서 목록에 review_job 메타 아이템이 섞여 보인다.
    # 소유자 정보는 필요하므로 owner_user_id로 옮겨 저장한다.
    if "user_id" in item:
        owner = item.pop("user_id")
        if owner and not item.get("owner_user_id"):
...
def _load_review_job(review_job_id: str) -> dict | None:
    ...
    if not isinstance(item, dict):
        return None
    # 과거 데이터 호환: owner_user_id를 user_id로도 노출.
    if item.get("owner_user_id") and not item.get("user_id"):
...
def _create_review_job(doc_id: str, user_id: str, baseline_result: dict, doc_dict: dict) -> dict:
    ...
    # LLM/hybrid 규칙은 항상 AgentCore Reviewer가 판단해야 한다.
    # (과거에는 baseline이 NOT_CHECKED이고 evidence_found가 비어 있으면 호출 대상에서 제외했지만,
    #  그러면 LLM 전용 규칙이 실제로는 한 번도 LLM을 거치지 않는 문제가 생긴다.)
    agent_rules = [
        rule for rule in enabled_rules
        if str(rule.get("evaluation_type", "llm")) in {"llm", "hybrid"}
    ]
    static_rule_count = sum(
        1 for rule in enabled_rules
        if str(rule.get("evaluation_type", "llm")) == "static"
    )
    _log(
        "create_review_job", "info",
        doc_id=doc_id,
        review_job_id=review_job_id,
        enabled_rule_count=len(enabled_rules),
        agent_rule_count=len(agent_rules),
        static_rule_count=static_rule_count,
    )
...
def _enqueue_agentcore_review(job: dict) -> bool:
    review_job_id = str(job.get("review_job_id", ""))
    if not job.get("enabled_rules"):
        _log(
            "enqueue_agentcore_review", "info",
            review_job_id=review_job_id,
            skipped="no_agent_rules",
        )
        return False
    function_name = os.environ.get("AWS_LAMBDA_FUNCTION_NAME", "")
    if not function_name:
        _log(
            "enqueue_agentcore_review", "warn",
            review_job_id=review_job_id,
            skipped="AWS_LAMBDA_FUNCTION_NAME not set (likely local/test env)",
        )
        return False
    payload = {
    ...
    try:
        ...
        _log(
            "enqueue_agentcore_review", "error",
            review_job_id=review_job_id,
    ...
    _log(
        ...
        agent_rule_count=len(job.get("enabled_rules") or []),
        status="queued",
    )
    return True
...
def _handle_async_agentcore_review(event: dict) -> dict:
    """Fire-and-forget invocation of AgentCore Reviewer.

    The Runtime persists the final merged result and status directly to
    DynamoDB (``review_job#<id>``), so this Lambda only needs to kick the
    Runtime off and return. A short read timeout is used so the Lambda does
    not hold the HTTP connection open for the duration of the Bedrock loop.
    A ``ReadTimeoutError`` from the short timeout is treated as the expected
    "Runtime accepted the job" signal.
    """
    review_job_id = str(event.get("review_job_id", ""))
    job = _load_review_job(review_job_id)
    if not job:
        _log(
            "async_agentcore_review", "error",
            review_job_id=review_job_id,
    ...
    payload = {
        ...
        "review_job_table": TABLE_NAME,
        "review_job_pk": _review_job_key(review_job_id),
    }
    try:
        _get_agentcore_runtime_ff_client().invoke_agent_runtime(
            agentRuntimeArn=runtime_arn,
            contentType="application/json",
            accept="application/json",
            payload=_json(payload).encode("utf-8"),
        ...
        _log(
            "async_agentcore_review", "info",
            review_job_id=review_job_id,
            stage="runtime_completed_inline",
        )
        return {"status": "ok", "review_job_id": review_job_id}
    except Exception as exc:
        # ReadTimeoutError (from botocore) means the Runtime accepted the
        # request and is still processing when the short read timeout fires.
        # This is the expected happy path for fire-and-forget reviews: the
        # Runtime itself writes the final result to DynamoDB.
        exc_name = type(exc).__name__
        if exc_name in {"ReadTimeoutError", "ConnectTimeoutError", "ReadTimeout"}:
            _log(
                "async_agentcore_review", "info",
                review_job_id=review_job_id,
                stage="dispatched_runtime_running",
                note="short read timeout, runtime will persist result",
            )
            return {"status": "ok", "review_job_id": review_job_id, "dispatched": True}

        job = _load_review_job(review_job_id) or job
        job["status"] = "failed"
...
def _handle_get_review_result(doc_id: str, review_job_id: str, event: dict) -> dict:
    ...
    # If the Runtime has written an agent_result but the merged_result is not
    # yet computed (fire-and-forget path), compute the merge lazily here so the
    # frontend receives consistent rule_evaluations/summary/categories/score.
    merged_result = job.get("merged_result")
    if not merged_result and isinstance(job.get("agent_result"), dict):
        merged_result = _merge_agentcore_review_result(
            job.get("baseline_result", {}),
            job.get("agent_result", {}),
        )
        # Persist the computed merge so subsequent reads are cheap.
        try:
            job["merged_result"] = merged_result
            job["rule_evaluations"] = merged_result.get("rule_evaluations", [])
            _save_review_job(job)
        except Exception as exc:
            _log(
                "get_review_result", "warn",
                review_job_id=review_job_id,
                persist_merge_error=_safe_error_reason(exc),
            )

    result = deepcopy(merged_result or job.get("baseline_result") or {})
...
def _normalize_review_rule(rule: dict, *, custom: bool, user_id: str = "system") -> dict:
    ...
    # user_id 필드가 rule에 섞여 있으면 `user_id-updated_at-index` GSI에 올라가
    # 사이드바 문서 목록을 오염시킨다. 소유자 정보는 created_by/updated_by로만 둔다.
...
def _handle_list_documents(event: dict) -> dict:
    ...
    for item in owned_resp.get("Items", []):
        ...
        if not doc_id or doc_id in seen_ids:
            continue
        # 같은 테이블에 저장된 내부 메타 아이템(review_job, review_rule 등)은
        # 실제 문서가 아니므로 사이드바 목록에서 제외한다.
        if not str(doc_id).startswith("doc-"):
            continue
        if item.get("item_type") in {"review_job", _REVIEW_RULE_CUSTOM_TYPE, _REVIEW_RULE_OVERRIDE_TYPE}:
            continue
        seen_ids.add(doc_id)
        items.append({
            "document_id": doc_id,
...
# S3 bucket for architecture artefacts (.drawio + preview PNG).
# Default matches the value set by Terraform (ARTIFACTS_BUCKET env var).
ARTIFACTS_BUCKET = os.environ.get("ARTIFACTS_BUCKET", "") or os.environ.get("S3_BUCKET", "")

# Max upload sizes (bytes). Conservative so a Lambda 256MB invocation stays safe.
_MAX_DRAWIO_BYTES = 2 * 1024 * 1024     # 2 MB of XML is already very large
_MAX_PREVIEW_BYTES = 8 * 1024 * 1024    # 8 MB PNG
...
def _s3_put_object(key: str, body: bytes, content_type: str) -> None:
    if not ARTIFACTS_BUCKET:
        raise RuntimeError("ARTIFACTS_BUCKET not configured")
...
def _s3_presigned_get(key: str, expires: int = 3600) -> str:
    ...
    try:
        ...
        _log("s3_presign", "warn", key=key, error=_safe_error_reason(exc))
        return ""
...
# ---------------------------------------------------------------------------
# Architecture: .drawio upload + PNG preview upload (viewer.diagrams.net flow)
# ---------------------------------------------------------------------------

def _decode_base64_payload(raw: str) -> bytes:
    """Decode a base64 or data-URL base64 payload into bytes.

    Accepts both ``data:image/png;base64,<b64>`` and bare base64 strings.
    Raises ``ValueError`` if the payload is not decodable.
    """
    import base64
    ...
    text = "".join(text.split())  # strip whitespace/newlines
    try:
        return base64.b64decode(text, validate=False)
    except Exception as exc:  # noqa: BLE001
        raise ValueError(f"invalid base64: {_safe_error_reason(exc)}") from exc


def _set_architecture_artifact_urls(
    doc_id: str,
    item: dict,
    *,
    drawio_key: str | None = None,
    drawio_url: str | None = None,
    preview_key: str | None = None,
    preview_url: str | None = None,
    drawio_uploaded_at: str | None = None,
    preview_uploaded_at: str | None = None,
) -> tuple[dict, dict]:
    """Write artefact URLs into sections.architecture and save the document.

    Returns ``(saved_item, arch_section_after)``.
    """
    expected_version = int(item.get("version", 0))
    doc = json.loads(_json(item))
    sections = doc.setdefault("sections", {})
    arch = sections.setdefault("architecture", {})
    if not isinstance(arch, dict):
        arch = {}
        sections["architecture"] = arch

    if drawio_key is not None:
        arch["drawio_s3_key"] = drawio_key
    if drawio_url is not None:
        arch["drawio_url"] = drawio_url
    if preview_key is not None:
        arch["preview_s3_key"] = preview_key
    if preview_url is not None:
        arch["preview_url"] = preview_url
    if drawio_uploaded_at is not None:
        arch["drawio_uploaded_at"] = drawio_uploaded_at
    if preview_uploaded_at is not None:
        arch["preview_uploaded_at"] = preview_uploaded_at

    saved = _conditional_save_document(doc, expected_version)
    return saved, arch


def _handle_architecture_upload(doc_id: str, body: dict, event: dict) -> dict:
    """POST /documents/{id}/architecture/upload.

    Body (JSON):
        {"filename": "architecture.drawio", "content": "<raw XML>"}
    """
    _user_id, permission, item, err = _load_document_for_action(doc_id, event, "edit")
    if err:
        return err
    if not ARTIFACTS_BUCKET:
        return _failed(500, error_reason="ARTIFACTS_BUCKET not configured")

    filename = str(body.get("filename") or "architecture.drawio")
    content = body.get("content") or ""
    if not isinstance(content, str) or not content.strip():
        return _failed(400, error_reason="missing drawio content",
                       message="업로드할 .drawio XML이 비어 있습니다.")

    xml_bytes = content.encode("utf-8")
    if len(xml_bytes) > _MAX_DRAWIO_BYTES:
        return _failed(413, error_reason="drawio payload too large",
                       message=f".drawio 파일 크기가 {_MAX_DRAWIO_BYTES} bytes를 초과합니다.")

    # Very light sanity check — accept both .drawio and .xml contents.
    head = content.lstrip()[:200].lower()
    if not (head.startswith("<?xml") or "<mxfile" in head or "<mxgraphmodel" in head):
        return _failed(400, error_reason="not a drawio xml",
                       message=".drawio 또는 mxGraph XML 형식이 아닙니다.")

    now = _now_iso()
    key = f"docs/{doc_id}/diagrams/architecture.drawio"
    try:
        _s3_put_object(key, xml_bytes, "application/xml")
    except Exception as exc:  # noqa: BLE001
        _log("architecture_upload", "error", doc_id=doc_id, error=_safe_error_reason(exc))
        return _failed(500, error_reason="s3 put_object failed",
                       message="S3 업로드에 실패했습니다.")

    url = _s3_presigned_get(key)

    try:
        saved, _ = _set_architecture_artifact_urls(
            doc_id, item,
            drawio_key=key, drawio_url=url,
            drawio_uploaded_at=now,
        )
    except VersionConflictError:
        return _failed(409, error_reason="version_conflict",
                       message="문서 버전이 변경되었습니다. 다시 시도해주세요.")
    except Exception as exc:  # noqa: BLE001
        _log("architecture_upload", "error", doc_id=doc_id, error=_safe_error_reason(exc))
        return _failed(500, error_reason="save failed",
                       message="문서 저장에 실패했습니다.")

    # Notify subscribers that the artefact URL fields changed.
    _publish_document_patch(
        doc_id,
        [
            {"op": "replace", "path": "/sections/architecture/drawio_s3_key", "value": key},
            {"op": "replace", "path": "/sections/architecture/drawio_url", "value": url},
            {"op": "replace", "path": "/sections/architecture/drawio_uploaded_at", "value": now},
        ],
        saved,
    ...
    _log("architecture_upload", "info", doc_id=doc_id, bytes=len(xml_bytes), filename=filename)
    return _ok({
        "document_id": doc_id,
        "permission": permission,
        "filename": filename,
    ...
    }, message=".drawio 파일이 업로드되었습니다.")


def _handle_architecture_preview_upload(doc_id: str, body: dict, event: dict) -> dict:
    """POST /documents/{id}/architecture/preview.

    Body (JSON):
        {"png_base64": "<base64 or data:image/png;base64,...>"}
    """
    _user_id, permission, item, err = _load_document_for_action(doc_id, event, "edit")
    if err:
        return err
    ...
    raw = body.get("png_base64") or body.get("content") or ""
    try:
        png_bytes = _decode_base64_payload(raw)
    except ValueError as exc:
        return _failed(400, error_reason=str(exc),
                       message="PNG base64 디코딩에 실패했습니다.")

    if len(png_bytes) > _MAX_PREVIEW_BYTES:
        return _failed(413, error_reason="png payload too large",
                       message=f"PNG 크기가 {_MAX_PREVIEW_BYTES} bytes를 초과합니다.")
    if not png_bytes.startswith(b"\x89PNG\r\n\x1a\n"):
        return _failed(400, error_reason="not a png",
                       message="PNG 파일이 아닙니다.")

    now = _now_iso()
    key = f"docs/{doc_id}/diagrams/architecture.png"
    try:
        _s3_put_object(key, png_bytes, "image/png")
    except Exception as exc:  # noqa: BLE001
        _log("architecture_preview_upload", "error", doc_id=doc_id, error=_safe_error_reason(exc))
        return _failed(500, error_reason="s3 put_object failed",
                       message="프리뷰 저장에 실패했습니다.")

    url = _s3_presigned_get(key)
    try:
        saved, _ = _set_architecture_artifact_urls(
            doc_id, item,
            preview_key=key, preview_url=url,
            preview_uploaded_at=now,
        )
    except VersionConflictError:
        return _failed(409, error_reason="version_conflict",
                       message="문서 버전이 변경되었습니다. 다시 시도해주세요.")
    ...
    _publish_document_patch(
        doc_id,
        [
            {"op": "replace", "path": "/sections/architecture/preview_s3_key", "value": key},
            {"op": "replace", "path": "/sections/architecture/preview_url", "value": url},
            {"op": "replace", "path": "/sections/architecture/preview_uploaded_at", "value": now},
        ],
        saved,
        int(item.get("version", 0)),
        "architecture_preview_upload",
    )

    _log("architecture_preview_upload", "info", doc_id=doc_id, bytes=len(png_bytes))
    return _ok({
        "document_id": doc_id,
        "permission": permission,
        "preview_s3_key": key,
...
def _send_share_invitation_email(
    *,
    to_email: str,
    doc_title: str,
    doc_id: str,
    ...
    """공유 초대 이메일 발송. 실패해도 조용히 상태만 반환 (초대 자체는 성공시킴)."""
    if not SES_FROM_EMAIL:
        return {"sent": False, "reason": "from_email_not_configured"}

    client = _get_ses_client()
    role_label = "편집자" if role == "edit" else "뷰어"
    pending_note = (
        '<p style="color:#6b7280;font-size:13px;margin:16px 0 0;">'
        '아직 Doc Agent에 가입하지 않으셨네요. 링크를 통해 먼저 가입해주시면 '
        '문서가 자동으로 목록에 추가됩니다.'
        '</p>'
        if target_status == "pending"
        else ""
    )
    open_url = SHARE_APP_URL or "https://mzc-spaceflow.com/"

    safe_title = _html_escape(doc_title or "제목 없음")
    safe_inviter = _html_escape(inviter_email or "동료")

    subject = f"[Doc Agent] {inviter_email or '동료'}님이 문서를 공유했습니다 — {doc_title or '제목 없음'}"
    html_body = f"""
    <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;color:#111827;max-width:560px;margin:0 auto;padding:24px;">
      <h2 style="margin:0 0 12px;font-size:18px;">문서 공유 안내</h2>
      <p style="margin:0 0 16px;line-height:1.6;">
        <strong>{safe_inviter}</strong> 님이 Doc Agent 문서를 공유했습니다.
      </p>
      <table style="background:#f9fafb;border:1px solid #e5e7eb;border-radius:8px;padding:12px 16px;margin:0 0 16px;width:100%;border-collapse:collapse;">
        <tr><td style="padding:4px 0;color:#6b7280;font-size:12px;">문서</td><td style="padding:4px 0;font-size:14px;font-weight:600;">{safe_title}</td></tr>
        <tr><td style="padding:4px 0;color:#6b7280;font-size:12px;">권한</td><td style="padding:4px 0;font-size:14px;">{role_label}</td></tr>
      </table>
      <p style="margin:0 0 24px;">
        <a href="{open_url}"
           style="display:inline-block;background:#0057ff;color:#fff;padding:10px 18px;border-radius:6px;text-decoration:none;font-weight:600;font-size:14px;">
          Doc Agent 열기
        </a>
      </p>
      {pending_note}
      ...
      <p style="color:#9ca3af;font-size:12px;margin:0;">
        이 메일은 Doc Agent 공유 기능을 통해 자동 발송되었습니다.
      </p>
    </div>
    """
    text_body = (
        f"{inviter_email or '동료'}님이 문서를 공유했습니다.\n\n"
        f"문서: {doc_title or '제목 없음'}\n"
        f"권한: {role_label}\n\n"
        f"열기: {open_url}\n"
    )
    ...
    try:
        ...
        print(f"[share] ses send failed to={to_email}: {exc}")
        return {"sent": False, "reason": _safe_error_reason(exc)}
...
def _handle_create_share(doc_id: str, body: dict, event: dict) -> dict:
    ...
    if target_sub:
        ...
        email_result = _send_share_invitation_email(
            to_email=target_email or email,
            doc_title=doc_item.get("title", "제목 없음"),
            doc_id=doc_id,
            role=role,
            inviter_email=shared_by_email,
...
def handler(event: dict, context: Any) -> dict:
    ...
    try:
        ...
        elif method == "POST" and action == "architecture" and len(parts) >= 4:
            # /documents/{id}/architecture/upload  — .drawio XML 업로드
            # /documents/{id}/architecture/preview — viewer에서 추출한 PNG 업로드
            sub = parts[3]
            body = json.loads(event.get("body", "{}"))
            if sub == "upload":
                return _handle_architecture_upload(doc_id, body, event)
            if sub == "preview":
                return _handle_architecture_preview_upload(doc_id, body, event)
            return _response(400, {"error": f"unknown architecture sub-action: {sub}"})
