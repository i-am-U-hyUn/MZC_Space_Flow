...
class ExecutiveSummarySection(BaseModel):
    ...
    Contains both v2 schema fields (customer_intro, problem_statement, etc.)
    and the legacy `groups` structure used by the frontend editor and
    `executive_summary_groups` export context key.
    """
    model_config = {"extra": "forbid"}

    # v2 structured fields (patched by discovery/orchestrator, rendered by export_docx)
    ...
    # Frontend category-group editor state (rendered as executive_summary_groups)
...
class ArchitectureSection(BaseModel):
    ...
    Runtime diagram metadata (drawio_*, preview_*) is persisted here so that
    the Lambda handler's .drawio upload flow and the frontend preview share a
    single source of truth. These fields are optional strings and are not part
    of the DOCX template export context.
    """
    model_config = {"extra": "forbid"}
    ...
    # Runtime diagram metadata (persisted, not in template export context)
    drawio_s3_key: Optional[str] = None
    drawio_url: Optional[str] = None
    drawio_uploaded_at: Optional[str] = None
    preview_s3_key: Optional[str] = None
    preview_url: Optional[str] = None
    preview_uploaded_at: Optional[str] = None
